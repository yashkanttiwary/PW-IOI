...
export default function APIKeyGate() {
  ...
  const [model, setModel] = useState(state.aiConfig?.model || 'gemini-2.5-pro');
  ...
  const saveConfig = () => {
    ...
    dispatch({
      type: 'SET_AI_CONFIG',
      payload: { apiKey: trimmed, model: model.trim() || 'gemini-2.5-pro' },
    });
  ...
  const testConnection = async () => {
    ...
    const result = await callAI({
      module: 'dashboard',
      apiKey: trimmed,
      model: model.trim() || 'gemini-2.5-pro',
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-[#0D0D0D] border border-[#333] rounded-lg px-4 py-3 text-white focus:border-[#00F5FF] outline-none"
              />
              <p className="text-xs text-[#A0A0A0] mt-2">Example: gemini-2.5-pro or any Gemini model available on your key.</p>
            </div>

            {error && <div className="text-sm text-[#FF9100]">{error}</div>}
